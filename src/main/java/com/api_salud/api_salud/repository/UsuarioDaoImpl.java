...
public class UsuarioDaoImpl implements UsuarioDao {
    ...
    public UsuarioContextoResponse usuarioObtenerDatosContextoPorUsername(String username) {
        try {
            SqlParameterSource param = new MapSqlParameterSource().addValue("p_username", username);
            Map<String, Object> out = callObtenerDatosJwtPorUsername.execute(param);
            List<UsuarioContextoResponse> list = (List<UsuarioContextoResponse>) out.get("o_usuario");
            return (list != null && !list.isEmpty()) ? list.get(0) : null;
        } catch (Exception e) {
            log.error("Error al leer Obtener Datos Jwt Por Username: ", e);
            return null;
        }
    }
